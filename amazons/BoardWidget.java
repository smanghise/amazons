...
import static amazons.Square.SQUARES;
...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        g.drawLine(0, 0, 0, BOARD_SIDE);
        g.drawLine(0, 0, BOARD_SIDE, 0);
        g.drawLine(0, BOARD_SIDE - 1, BOARD_SIDE - 1, BOARD_SIDE - 1);
        g.drawLine(BOARD_SIDE - 1, BOARD_SIDE - 1, BOARD_SIDE - 1, 0);
        ...
        for (int i = 0; i <= Board.SIZE * Board.SIZE - 1; i++) {
            ...
            int row = pos.row() * SQUARE_SIDE;
            int col = pos.col() * SQUARE_SIDE;

            if (lightSquare) {
                g.setColor(LIGHT_SQUARE_COLOR);
                g.fill3DRect(col, row, SQUARE_SIDE, SQUARE_SIDE, false);
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int index = 0; index < Board.SIZE * Board.SIZE - 1; index++) {
            if (sq(SQUARES[index].toString()).getPiece().equals(BLACK)) {
                drawQueen(g, sq(SQUARES[index].toString()), Piece.BLACK);
            }

            if (sq(SQUARES[index].toString()).getPiece().equals(WHITE)) {
                drawQueen(g, sq(SQUARES[index].toString()), Piece.WHITE);
            }

            if (sq(SQUARES[index].toString()).getPiece().equals(SPEAR)) {
                drawSpear(g, sq(SQUARES[index].toString()));
            }
        }
    }
    ...
    private void click(Square s) {
        System.out.println("Square Clicked at : " + s.col() + " : " + s.row());

        boardMoves.add(s);
        if (boardMoves.size() == 3) {
            String newMove = String.format("%s-%s(%s)",
                    boardMoves.get(0).toString(),
                    boardMoves.get(1).toString(), boardMoves.get(2).toString());
            System.out.println("New Move : " + newMove);
            boardMoves.clear();
            try {
                _commands.put(newMove);
            } catch (InterruptedException ie) {
                System.out.println("Caught Mouse Click error");
            }
        }
    ...
    /** square. */
    private List<Square> boardMoves = new ArrayList<Square>();
